...
export function RecallLine({ data, isMulticlass = false }) {
  ...
  const chartData = data.map(m => {
    if (isMulticlass) {
      return {
        name: m.name,
        normal: m.normal?.recall ?? m.overall.recall,
        dos: m.DoS?.recall ?? m.overall.recall,
        probe: m.Probe?.recall ?? m.overall.recall,
        r2l: m.R2L?.recall ?? m.overall.recall,
        u2r: m.U2R?.recall ?? m.overall.recall,
      };
    }
    return {
      name: m.name,
      normal: m.normal?.recall ?? m.overall.recall,
      attack: m.attack?.recall ?? m.overall.recall
    };
  });
